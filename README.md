With these two experiments, I explore Transfer Learning using TensorFlow.js to train an existing model on how to 1) recognize the six sides of a dice and to 2) recognize random scribbles.
...
The demos show how to leverge a KNN classifier that can be trained using webcam captured images.  The captured image from the webcam is  processed by an activation of MobileNet. This network is already trained to recognize many classes from the popular ImageNet dataset, and is optimized to be compact, making it deployable in a browser.
